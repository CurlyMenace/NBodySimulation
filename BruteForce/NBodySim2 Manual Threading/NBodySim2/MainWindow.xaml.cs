...
namespace NBodySim2
{
    ...
    public partial class MainWindow : Window
    {
       ...
       DispatcherTimer uiTimer;
        ...
        private void threadForce(int N)
        {
            //Dynamically creates threads and assigns workload to them
            ...
            for(int i =0; i<numOThreads; i++)
            {
                ...
                int end = (N / numOThreads) + start;

                Thread thread = new Thread(() => addForce(N, start, end));
                thread.Start();
                threads.Add(thread);
            }
        ...
        public void addForce(int N, int start, int end)
        {
            //Used to add the force to each and every body based on other bodies

            //Splitting the work on either first or second forloop
